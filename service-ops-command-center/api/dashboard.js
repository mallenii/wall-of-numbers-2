...
function mockData(){
  ...
  return {
    ...
    mode:'mock',
    kpis:{firft:92.9, repeatRepair:715, roDuration:2, powertrainRate:4.99, endpointTech:48681, activeTech:49329, techLeft:612, uncertified:21394, flexHours:811038.3, distinctClaims:59677, ota:2233, fsa:614, other:0, customerPaid:640733, warranty:78815},
    trend,
...
function getBigQueryClient(){
  ...
  if(raw){
    ...
    return new BigQuery({ projectId: projectId || credentials.project_id, credentials });
  }
...
export default async function handler(req, res) {
  ...
  if (useMock) return res.status(200).json(mockData());

  try {
    const bq = getBigQueryClient();
    const runQuery = async (name) => {
      const [rows] = await bq.query({ query: queries[name], location: process.env.BIGQUERY_LOCATION || 'US' });
      return rows;
    };
    ...
    const data = mockData();
    data.mode='bigquery';
    data.kpis = {...data.kpis, firft: surveys ? +(firftYes/surveys*100).toFixed(2) : 0, roDuration:+avgDuration.toFixed(2), powertrainRate:+pwr.toFixed(2), endpointTech:Number(lt.end_point_tech||0), activeTech:Number(lt.n_current_tech_count_summarized_from_tech_table||0), techLeft:Number(lt.Defect_count_summarized_from_tech_table||0), uncertified:Number(lt.Current_uncertified_count||0), flexHours:Number(lf.Grand_Total_Flex_Time_Hours||0)};
